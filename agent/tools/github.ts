...
export const githubNotifications = new Graph(z.object({}))
  .meta({
...
export const githubPrs = new Graph(
  z.object({
  ...
  .run(
    (f) =>
      f.evaluate({
        fn: async (input: { repo: string; state?: string }) => {
          if (!GITHUB_TOKEN) {
...
export const githubIssues = new Graph(
  z.object({
    repo: z.string().describe('Repository in owner/name format'),
    query: z
      .string()
  ...
  .run(
    (f) =>
      f.evaluate({
        fn: async (input: { repo: string; query?: string; state?: string }) => {
          if (!GITHUB_TOKEN) {
